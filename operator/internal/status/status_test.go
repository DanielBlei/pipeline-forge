...
import (
	...
	v1alpha1 "github.com/DanielBlei/pipeline-forge/operator/api/v1alpha1"
)
...
var _ = Describe("Status Update", func() {
	Context("When updating status", func() {
		...
		It("should successfully update the status", func() {
			...
			resource.Status.SetTriggerStatus(v1alpha1.ObjectConditionCompleted)
			...
			Expect(err).NotTo(HaveOccurred())
			Expect(updatedObj.Status.Status).To(HaveValue(Equal(v1alpha1.ObjectConditionCompleted)))
		})
	})
})
